...
export default function Deploy() {
    ...
    const deployPair = async (option0: string, option1: string, address: string) => {
        ...
        let contract = await tronWeb.contract(FactoryABI, 'TJ8sRynQEQqu9xF9R4RQzLxG1YTryZ8Sed');
        const result = await contract.createPair(option0, option1, address).send();
        console.log(result);
        let pairsN = await contract.allPairsLength().call();
  ...
  return (
    <>
      <Center> 
        ...
        <Text>{address}</Text> 
        <TextInput label="First Option" value={firstOption} onChange={(event) => setFirstOption(event.currentTarget.value)} />
        <TextInput label="Second Option" value={secondOption} onChange={(event) => setSecondOption(event.currentTarget.value)} />
        <TextInput label="Reserve Token" value={tokenAddress} onChange={(event) => setTokenAddress(event.currentTarget.value)} />
        <Button
          onClick={() => {
